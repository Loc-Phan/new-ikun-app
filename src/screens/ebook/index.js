...
import { ListEbook } from '../../component/list';
...
class Ebook extends Component {
  ...
  async componentDidMount() {
    const categories = await NewClient.getEbooksCategories();
  ...
  async getData() {
    ...
    if (categorySelect.length > 0) {
      param.category_id = categorySelect;
    }
    const responses = await NewClient.getEbooks(param);
    if (responses?.data && responses?.data?.products) {
      const response = responses.data.products;
      const newData = [];
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <View style={[styles.header1]}>
            ...
            <Text style={styles.title}>{t('ebook.title')}</Text>
            <TouchableOpacity
              style={styles.viewSearch}
              onPress={() =>
                navigation.navigate('EbookSearchScreen', {
                  keySearch,
                })
              }
            >
