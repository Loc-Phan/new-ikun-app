...
export default class VideoPlayer extends Component {
  ...
  _toggleFullscreen() {
    ...
    this.state.changeFullscreen(state.isFullscreen);
  ...
  renderPlaySpeed() {
    const {openPlaySpeed} = this.state;
    return (
      <View style={{position: 'relative'}}>
        <TouchableWithoutFeedback
          style={{position: 'relative'}}
          onPress={() => this.setState({openPlaySpeed: !openPlaySpeed})}>
          <IconM
            name="play-speed"
        ...
        {openPlaySpeed && (
          <View
            ...
            {playSpeeds.map(({value, text}) => (
              <TouchableOpacity
                key={value}
                onPress={() => {
                  this.setState({rate: value, openPlaySpeed: false});
                }}>
                <Text
                  style={{
                    fontSize: 12,
